    ...
    void Update()
    {
        ...
        if((horizontalInput>0f || horizontalInput<0f) &&enableKey==true){
            if(pushing==true){
                ...
                pushingAnimation=false;
                AudioManager.instance.PlaySoundEffect(AudioManager.SoundEffect.PlayerRunning);
            }
        }
        ...
        {
            Instantiate(Bullet, gameObject.transform.position, Quaternion.identity).GetComponent<BulletBehaviour>().Shoot(gameObject, bulletDistance);
        }
